...
_APOLLO_WORKSPACE_DIR = os.getcwd()
_BUILDTOOL_ROOT_DIR = os.path.join(
...
def inside_docker():
    """
    Check if the env is in container
    """
...
def write_build_var_to_bazelrc(bazel_config_name, option_name):
    """
    write build variable to bazelrc
    """
...
def get_python_path(environ_cp, python_bin_path):
    ...
    for path in all_paths:
        if os.path.isdir(path) and not path.startswith(
                os.path.join(_APOLLO_WORKSPACE_DIR, "bazel-bin")):
            paths.append(path)
    return paths
...
def setup_common_dirs(environ_cp):
    ...
    cache_dir = os.path.join(_APOLLO_WORKSPACE_DIR, '.cache')
    dist_dir = os.path.join(_APOLLO_WORKSPACE_DIR, '.cache/distdir')
...
def setup_cuda_family_config_interactively(environ_cp):
    """setup cuda config based on user input"""
...
def main():
    ...
    _APOLLO_BAZELRC = os.path.join(_APOLLO_WORKSPACE_DIR, _APOLLO_BAZELRC)
    ...
    if reset_apollo_bazelrc() == _EXIT_SUCCESS:
        return _EXIT_SUCCESS
