...
from core.version_decide.mixology.incompatibility_cause import ConflictCause


class SolverFailure(Exception):
    """solve failure"""
    def __init__(self, incompatibility):  
...
class _Writer:
    ...
    def _is_collapsible(self, incompatibility):  
        # type: (Incompatibility) -> bool
    ...
    def _is_single_line(self, cause):  
        # type: (ConflictCause) -> bool
        return not isinstance(cause.conflict.cause, ConflictCause) and not isinstance(
            cause.other.cause, ConflictCause
        )

    def _count_derivations(self, incompatibility):  
        # type: (Incompatibility) -> None
