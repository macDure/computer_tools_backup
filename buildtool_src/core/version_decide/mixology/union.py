...
import core.version_decide.mixology.range
...
class Union(object):
    ...
    def of(cls, *ranges):
        ...
        if not flattened:
            return core.version_decide.mixology.range.EmptyRange()

        if any(constraint.is_any() for constraint in flattened):
            return core.version_decide.mixology.range.Range()
    ...
    def allows_all(self, other):  
        """# type: (_Union[core.version_decide.mixology.range.Range, Union]) -> bool"""
    ...
    def intersect(self, other):  
        """# type: (_Union[core.version_decide.mixology.range.Range, Union]) 
        # -> _Union[core.version_decide.mixology.range.Range, Union]"""
        our_ranges = iter(self._ranges)
        their_ranges = iter(self._ranges_for(other))
        new_ranges = []
    ...
    def excludes_single_version(self):  
        """# type: () -> bool"""
        difference = self.difference(core.version_decide.mixology.range.Range())

        return (
            isinstance(difference, core.version_decide.mixology.range.Range)
            and difference.is_single_version()
        )

    def _ranges_for(
        self, constraint
    ):  # type: (_Union[Union, core.version_decide.mixology.range.Range]) -> List[core.version_decide.mixology.range.Range]
        if constraint.is_empty():
            return []
    ...
    def __str__(self):
        if self.excludes_single_version():
            return "!={}".format(core.version_decide.mixology.range.Range().difference(self))
