...
from core.version_decide.mixology.failure import SolverFailure
from core.version_decide.mixology.incompatibility import Incompatibility
from core.version_decide.mixology.incompatibility_cause import (
    ConflictCause,
    NoVersionsCause,
    RootCause,
)
from core.version_decide.mixology.package_source import PackageSource
from core.version_decide.mixology.partial_solution import PartialSolution
from core.version_decide.mixology.range import Range
from core.version_decide.mixology.result import SolverResult
from core.version_decide.mixology.set_relation import SetRelation
...
class VersionSolver:
    ...
    def solve(self):  
        ...
        logger.debug("Version solving took {:.3f} seconds.".format(time.time() - start))
        logger.debug("Tried {} solutions.".format(self._solution.attempted_solutions))
    ...
    def _propagate_incompatibility(
        ...
        logger.debug("derived: {}".format(unsatisfied.inverse))
    ...
    def _resolve_conflict(
        ...
        """
        logger.error("conflict: {}".format(incompatibility))
        ...
        while not incompatibility.is_failure():
            ...
            logger.error("{} thus: {}".format(bang, incompatibility))
    ...
    def _add_incompatibility(self, incompatibility):  # type: (Incompatibility) -> None
        logger.debug("fact: {}".format(incompatibility))
